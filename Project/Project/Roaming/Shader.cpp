...
void Shader::setTexArray1D(const string& varName, const vector<float>& tex)
{
	GLuint texID = 0;
	glActiveTexture(GL_TEXTURE0+maxTexUnit);
	glGenTextures(1, &texID);
	...
	glTexImage1D(GL_TEXTURE_1D, 0, GL_R32F, tex.size(), 0, GL_RED, GL_FLOAT, tex.data());
...
void Shader::setTexArray2D(const string& varName, const vector<float>& tex, int width)
{
	if(tex.size() < width)
		width = tex.size();
	...
	GLfloat *pixels = new GLfloat[width*height*4];
	memcpy(pixels, tex.data(), tex.size()*sizeof(float));
...
void Shader::setTexArray2D(const string& varName, const vector<vector<float>>& tex)
{
	GLfloat *pixels;
	GLuint texID = 0;
	glActiveTexture(GL_TEXTURE0+maxTexUnit);
	...
	for(unsigned i=0; i<height; i++)
	{
		memcpy(pixels+i*width*4, tex[i].data(), width*sizeof(float));
	}
	glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, width, height, 0, GL_RED, GL_FLOAT, pixels);
	free(pixels);
