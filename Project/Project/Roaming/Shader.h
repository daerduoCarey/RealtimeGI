...
public:
	...
	GLuint getAttribLoc(const string& progName, const string& varName) { return glGetAttribLocation(progName2progID[progName], varName.c_str()); }
	GLuint getAttribLoc(const string& varName) { return getAttribLoc(currentProgName, varName); }
	GLuint getUniformLoc(const string& progName, const string& varName) { return glGetUniformLocation(progName2progID[progName], varName.c_str()); }
	GLuint getUniformLoc(const string& varName) { return getUniformLoc(currentProgName, varName); }
	...
	void setTexArray1D(const string& varName, const vector<float>& tex);
	void setTexArray2D(const string& varName, const vector<vec4f>& tex, int width = 1);
	void setTexArray2D(const string& varName, const vector<float>& tex, int width = 1);
	void setTexArray2D(const string& varName, const vector<vector<vec4f>>& tex);
	void setTexArray2D(const string& varName, const vector<vector<float>>& tex);
